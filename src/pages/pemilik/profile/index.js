...
const OwnerProfile = ({ navigation }) => {
  ...
  const fetchData = async () => {
    try {
      const response = await axios.get("http://localhost:9000/api/owner");
  ...
  return (
    <View style={{ flex: 1, flexDirection: "column", gap: 15, paddingHorizontal: "3%", paddingVertical: "1%" }}>
      ...
      <View style={{ flexDirection: "row", justifyContent: "center", gap: 10 }}>
        <Image source={{ uri: `http://localhost:9000/public/images/${user.foto}`}} style={{ width: 100, height: 100, borderRadius: 50 }} />
      </View>
