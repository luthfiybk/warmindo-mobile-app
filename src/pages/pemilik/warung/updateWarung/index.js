...
const TambahWarung = ({navigation, route}) => {
    ...
    const pickImage = async (type) => {
        ...
        if (!result.canceled) {
            setWarung(prevData => ({ ...prevData, [type]: result.assets[0] }));
        }
    };
    ...
    const handleUpload = async () => {
        ...
        try {
            const response = await axios.put(`http://localhost:9000/api/warung/${idwarung}`, formData, {
                headers: {
            ...
            navigation.navigate('DetailWarung', {idwarung: idwarung})
        } catch (error) {
            console.error(error);
        ...
        <LinearGradient start={{ x: 0.0, y: 0.4 }} end={{ x: 0.5, y: 1.0 }} locations={[0, 1]} colors={["#162953", "#162953"]} style={{ flex: 1, flexDirection: "column" }}>
            ...
            <View style={{ flex: 1, backgroundColor: "#fff", marginTop: hp("1%")}}>
                <View style={{marginHorizontal: 10, marginVertical: 10, gap: 10}}>
                    ...
                    {warung.logo && warung.logo.uri ? (
                        <Image source={{ uri: warung.logo.uri }} style={{ width: 200, height: 200, justifyContent: 'center', alignSelf: 'center' }} />
                        ) : (
                        <Image source={{ uri: `http://localhost:9000/public/images/${warung.logo}` }} style={{ width: 200, height: 200, justifyContent: 'center', alignSelf: 'center' }} />
                    )}
                    ...
                    {warung.gambar && warung.gambar.uri ? (
                        <Image source={{ uri: warung.gambar.uri }} style={{ width: 200, height: 200, justifyContent: 'center', alignSelf: 'center' }} />
                        ) : (
                        <Image source={{ uri: `http://localhost:9000/public/images/${warung.gambar}` }} style={{ width: 200, height: 200, justifyContent: 'center', alignSelf: 'center' }} />
                    )}
                    ...
                    <TouchableOpacity onPress={handleUpload} style={{backgroundColor: '#162953', padding: 10, borderRadius: 5}}>
                        <Text style={{color: 'white', textAlign: 'center'}}>Update Warung</Text>
                    </TouchableOpacity>
                </View>
            </View>
        </LinearGradient>
